...
static void Win32ProcessPendingMessages(game_controller_input *KeyboardController)
{
    ...
    while (PeekMessageA(&Message, NULL, 0, 0, PM_REMOVE))
    {
        ...
        case WM_KEYUP: {
            ...
            // 0);??????????????????????????????????? float IsDown =
            // ((Message.lParam % (1 << 31)) == 0);idk y put theese hhere
            // ???????????????????????
...
static void Win32FillSoundBuffer(win32_sound_output *SoundOutput, DWORD BytesToLock,
                                 DWORD BytesToWrite, game_sound_output_buffer *SourceBuffer)
{
    ...
    if (SUCCEEDED(GlobalSecondaryBuffer->Lock(BytesToLock, BytesToWrite, &Region1, &Region1Size,
...
static void win32DebugDrawVertical(win32_offscreeen_buffer *Backbuffer, int X, int Top, int Bottom,
                                   uint32_t Color)
{
    if (Top <= 0)
    {
...
inline void win32DrawSoundBufferMarker(win32_offscreeen_buffer *Backbuffer,
                                       win32_sound_output *SoundOutput, float C, int PadX, int Top,
                                       int Bottom, DWORD Value, uint32_t Color)
{
...
static void win32DebugSyncDisplay(win32_offscreeen_buffer *Backbuffer, int MarkerCount,
...
void CatStrings(size_t SourceACount, char *SourceA, size_t SourceBCount, char *SourceB,
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
    ...
    CatStrings(OnePastLastSlash - EXEFileName, EXEFileName, sizeof(SourceGameCodeDLLFilename) - 1,
               SourceGameCodeDLLFilename, sizeof(SourceGameCodeDLLFullPath),
               SourceGameCodeDLLFullPath);

    char TempGameCodeDLLFilename[] = "handmade_temp.dll";
    char TempGameCodeDLLFullPath[MAX_PATH];
    CatStrings(OnePastLastSlash - EXEFileName, EXEFileName, sizeof(TempGameCodeDLLFilename) - 1,
               TempGameCodeDLLFilename, sizeof(TempGameCodeDLLFullPath), TempGameCodeDLLFullPath);
...
#define GameUpdateHz (MonitorRefreshHertz / 2)
    ...
    HWND WindowHandle = CreateWindowEx(
        0, WindowClass.lpszClassName, "Handmade", WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT,
