...
static debug_read_file_result DEBUGPlatformReadEntireFile(char* Filename) {
    ...
    if (FileHandle != INVALID_HANDLE_VALUE)
    {
        ...
        if (GetFileSizeEx(FileHandle, &FileSize)) {
            ...
            Result.Contents = VirtualAlloc(0, FileSize32, MEM_RESERVE|MEM_COMMIT, PAGE_READWRITE);//Virtual alloc is debug code (only used for large allocations)
...
static float DEBUGPlatformWriteEntireFile(char* Filename, uint32_t MemorySize, void* Memory) {
...
static void Win32ProcessKeyboardMessage(game_button_state* NewState, char IsDown ){
...
static void Win32ProcessPendingMessages(game_controller_input* KeyboardController) {
    ...
    while (PeekMessageA(&Message, NULL, 0, 0, PM_REMOVE)) {
        ...
        case WM_KEYUP:
        {
            ...
            //float IsDown = ((Message.lParam % (1 << 31)) == 0);idk y put theese hhere ???????????????????????
...
static void win32DisplayBufferInWindow(win32_offscreeen_buffer *Buffer, HDC DeviceContext, 
...
static void win32DebugDrawVertical(win32_offscreeen_buffer* Backbuffer, int X, int Top, int Bottom, uint32_t Color) {
...
inline void win32DrawSoundBufferMarker(win32_offscreeen_buffer* Backbuffer, win32_sound_output* SoundOutput, float C, int PadX, int Top, int Bottom, DWORD Value, uint32_t Color) {
...
static void win32DebugSyncDisplay(win32_offscreeen_buffer* Backbuffer, int MarkerCount, win32_debug_time_marker *Markers, win32_sound_output* SoundOutput, float TargetSecondsPerFrame)
{
    int PadX = 16;
    int PadY = 16;
...
)
{
    ...
    //This sets the windows scheduler granularity to 1ms so Sleep() can be more granular -> D18:53:20
    ...
    if (WindowHandle != NULL) {
...
#if 0   // This tests the PlayCursor/WriteCursor update frequency, on my machine -> 480
        while(true)
        {
            DWORD PlayCursor;
            DWORD WriteCursor;
        ...
        GameMemory.TransientStorage = ((uint8_t*)GameMemory.PermanentStorage + GameMemory.TransientStorageSize);
        ...
        if (Samples && GameMemory.PermanentStorage && GameMemory.TransientStorage) {
            ...
            while(Running) 
            {
                ...
                *NewKeyboardController = ZeroController;
                 ...
                 *
                 * We define a safety value that is the number of samples that we think our game update
                 * loop may vary by (let's say up to 2ms)
                 *
                 * When we wake up to write audio, we will look and see what the play cursor position is
                 * and we will forecast ahead what we think the play cursor will be after on the next 
                 * frame boundary.
                 *
                 * We will then look to see if the write cursor is before that by at least our safety
                 ...
                 * the audio perfectly, so we will write one frame's worth of audio plus the safety
                ...
                GameUpdateAndRender(&GameMemory, NewInput, &Buffer, &SoundBuffer); //NOTE(Bob):check declaration when u can             
...
//#if HANDMADE_INTERNAL
                    ...
                    char TextBuffer[256];
                    sprintf_s(TextBuffer, sizeof(TextBuffer), "LPC%u BTL%u TC%u BTW%u - PC%u WC%u DELTA%u (%fs)/n",
                              LastPlayCursor, BytesToLock, TargetCursor, BytesToWrite, PlayCursor, WriteCursor, AudioLatencyBytes,
                              AudioLatencySeconds);
                    OutputDebugStringA(TextBuffer);
//#endif
                ...
                float TestSecondsElapsedPerFrame = win32GetSecondsElapsed(LastCounter, win32GetWallClock());
                ...
                if (SecondsElapsedPerFrame < TargetSecondsPerFrame)
                {
                    if (SleepIsGranular)
                    {
                        ...
                        if (SleepMs > 0)  // i added the -1, something weird happening with the sleep calculations
                        {
                            Sleep(SleepMs); // same here
                        }
                    }
        
                    TestSecondsElapsedPerFrame = win32GetSecondsElapsed(LastCounter, win32GetWallClock());
...
//#if 0           
                ...
                char tempBuffer[256];
                sprintf_s(tempBuffer, sizeof(tempBuffer), "%f.02 ms/f; %f.02 f/s; %f.02 mc/f\n", MSPerFrame, FPS, MCPF);
